...
import {requestHtmlInterface} from "../requestHtmlInterface";
import {AdModelI, Ad} from "../../models/ad/AdModel";

const colors = require('colors');

export class Gumtree implements ProviderInterface {
    debug: boolean = false;
    $ = null;
    requestHtml: requestHtmlInterface;

    constructor(requestHtml: requestHtmlInterface, debug) {
    ...
    public getUrlAds(filter: FilterModelI):AdInterface[]{
        ...
        const featuredAds: AdInterface[] =  this.$('ul[data-q="featuredresults"] > li').map(extractElementAd).get();

        console.log('number of featuredAds:', featuredAds.length)

        const naturalAds:AdInterface[] = this.$('ul[data-q="naturalresults"] > li.natural').map(extractElementAd).get();
